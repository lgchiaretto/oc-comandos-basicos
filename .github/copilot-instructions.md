...
- **scripts/test-commands.sh**: Main test orchestrator script
- **scripts/generate-all-tests.py**: Python script to auto-generate test modules
- **scripts/cleanup-test-project.sh**: Script to cleanup test projects
...
3. Run the specific test module: `./scripts/test-commands.sh --module XX`
4. Fix any issues and run full suite: `./scripts/test-commands.sh`
...
To exclude specific code blocks from test generation, use the `(ignore-test)` marker:
````markdown
```bash ignore-test
...
### Running Tests
- All tests: `./scripts/test-commands.sh`
- Specific module: `./scripts/test-commands.sh --module 05`
- Verbose mode: `./scripts/test-commands.sh --verbose`
- Stop on first error: `./scripts/test-commands.sh --stop-on-error`
...
4. **Error Handling**
   ...
   - run ./scripts/test-commands.sh with no parameter to validate all changes (it will take time)
